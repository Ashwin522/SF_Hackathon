...
Captures defensive coordinates and gets counter-positioning recommendations from Claude
"""
...
Be specific with coordinates and player movements."""
    ...
    print("="*80)
    print("BASKETBALL TACTICAL ANALYSIS - MULTI-TURN CONVERSATION WITH LLM")
    print("="*80)
    ...
    while True:
        ...
        if terminated or frame >= 100:
            break
    
    print(f"Simulation complete! Captured {len(game_snapshots)} frames.\n")
    
    # Analyze selected frames with LLM
    selected_frames = [0, len(game_snapshots)//4, len(game_snapshots)//2, 3*len(game_snapshots)//4]
...
3. Specific counter-position recommendations for each defender
...
defensive weaknesses? Give specific player movements and positioning."""
